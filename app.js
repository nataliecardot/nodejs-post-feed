...
// Want to offer data from this server to clients served by a different server (front and back end are typically served on different servers); set CORS headers to override CORS error when different domain tries to send an HTTP request to this server. CORS (exchange of data between client and server [sending requests and responses] running on different domains [for example localhost:3000 and localhost:8080; port number is part of domain]) not allowed by browsers by default
